...
p0 <- ggtree::gheatmap(ggtree::ggtree(tr),df %>% as.data.frame %>% select(colistin_ns) %>% mutate_all(as.factor),colnames_position = 'top',width = .25,colnames_angle = 90,legend_title = 'Colistin non-susceptibility',hjust = 0,color = NULL) + ylim(NA,485) + feature_scale
p0
```
...
p0 <- ggtree::gheatmap(ggtree::ggtree(tr), genotype_mat %>% as.data.frame %>% mutate_all(as.factor) %>% `colnames<-`(paste0("Variant ",1:ncol(.))),colnames_position = 'top',width = 1.5,colnames_angle = 90,legend_title = 'Colistin non-susceptibility',hjust = 0,color = NULL) + ylim(NA,485) + feature_scale
p0 
```
