...
vignette: >
  %\VignetteIndexEntry{Phenotype and genotype association testing using ancestral state reconstruction}
...
p0 <- ggtree::gheatmap(ggtree::ggtree(tr),df %>% as.data.frame %>% select(colistin_ns) %>% mutate_all(as.factor),colnames_position = 'top',width = .25,low = 'white',high='black',colnames_angle = 90,legend_title = 'Colistin non-susceptibility',hjust = 0,color = NULL) + ylim(NA,485) + feature_scale
p0
```
...
paste0("Black indicates feature presence:") 
p0 <- ggtree::gheatmap(ggtree::ggtree(tr), genotype_mat %>% as.data.frame %>% select(colistin_ns) %>% mutate_all(as.factor) %>% `colnames<-`(NULL),colnames_position = 'top',width = 1,low = 'white',high='black',colnames_angle = 90,legend_title = 'Colistin non-susceptibility',hjust = 0,color = NULL) + ylim(NA,485) + feature_scale
p0 
```
...
asr_of_genotypes <- lapply(variables,FUN=function(x){
  phyloAMR::asr(df = genotype_mat,tr = tr,tip_name_variable = "isolate_no",trait = x,model = "ER",node_states = "joint") %>% .$parent_child_df
})
```

```{r,fig.height=9}
# Synchronous impact
synchronous_df <- lapply(asr_of_genotypes,phyloAMR::synchronous_transitions,asr_obj$parent_child_df) %>% do.call(rbind,.) %>% mutate(genotype = variables)   
...
```{r} 
downstream_df <- lapply(asr_of_genotypes,phyloAMR::downstream_transitions,asr_obj$parent_child_df,tr=tr,node_states='joint') %>% do.call(rbind,.) %>% mutate(genotype = variables)
