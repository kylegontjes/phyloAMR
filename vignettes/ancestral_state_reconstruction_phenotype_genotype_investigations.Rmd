...
For sake of time, we will only run the analysis on the first twelve genotypes in this matrix
```{r,echo=F,message=F}
genotype_mat$isolate_no <- rownames(genotype_mat)
variables <- colnames(genotype_mat %>% select(-isolate_no)) %>% .[1:12]
genotypes<- genotype_mat %>% .[match(tr$tip.label,.[['isolate_no']]),]

asr_of_genotypes <- pbmcapply::pbmclapply(variables,FUN=function(x){
