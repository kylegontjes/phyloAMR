...
p0 <- ggtree::gheatmap(ggtree::ggtree(tr),df %>% as.data.frame %>% select(colistin_ns) %>% mutate_all(as.factor),colnames_position = 'top',width = .25,low = 'white',high='black',colnames_angle = 90,legend_title = 'Colistin non-susceptibility',hjust = 0,color = NULL) + ylim(NA,450) + feature_scale
p0
```
...
ggtree::gheatmap(p0.1,asr_cluster_obj %>% as.data.frame %>% select(asr_cluster_renamed) %>% mutate_all(as.factor) ,colnames_position = 'top',width = .25,colnames_angle = 90,legend_title = 'Colistin non-usceptibility',hjust = 0,offset=0.0000075,color=NULL) + ylim(NA,500) + cluster_scale
```
