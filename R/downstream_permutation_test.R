...
#' @export
downstream_permutation_test <- function(comparitor,df,tr,tip_name_var,trait,node_states = "joint",num_permutations=1000,num_cores = 6){
  library(pbmcapply)
  # trait
  trait_asr = asr(df=df,tr=tr,tip_name_var=tip_name_var,pheno = trait,model = "ER",node_states = 'joint',conf_threshold = 0.875) %>% .$parent_child_df
  ...
  # Get observed
  trait_downstream_obs = downstream_gain_loss(comparitor_asr,trait_asr,tr)
  # permute
  ...
  asr_permutation = pbmclapply(permutation_names,FUN=function(x){
    asr_result = asr(df=trait_runs,tr=tr,tip_name_var="tip_name_var",pheno = x,model = "ER",node_states = 'joint',conf_threshold = 0.875) %>% .$parent_child_df
