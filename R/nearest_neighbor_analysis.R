...
#' @export
nearest_neighbor_analysis <- function(comparison_df, categorical_vars = NULL, binary_vars = NULL, continuous_vars = NULL, log_2 = FALSE) {
  ...
  }) %>% do.call(cbind, .) %>% as.data.frame %>% `colnames<-`(paste0(categorical_vars, "_cat_diff"))
